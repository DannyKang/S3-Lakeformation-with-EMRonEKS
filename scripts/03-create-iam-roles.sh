...
# 필수 환경 변수 확인
if [ -z "$ACCOUNT_ID" ] || [ -z "$REGION" ] || [ -z "$ICEBERG_BUCKET_NAME" ]; then
...
ROLES=(
    "LF_DataStewardRole:Data Steward - Full data access"
    "LF_GangnamAnalyticsRole:Gangnam Analyst - Gangnam district only"
    "LF_OperationRole:Operation Team - Operational data only"
    "LF_MarketingPartnerRole:Marketing Partner - Gangnam 20-30s only"
)
...
echo "   • IRSA 신뢰 정책은 05-setup-emr-on-eks.sh에서 추가됩니다"
echo ""
echo "✅ 다음 단계: ./scripts/04-setup-lakeformation-permissions-iceberg.sh"
